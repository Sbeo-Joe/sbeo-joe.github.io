  ...
  function createSearchModal() {
    ...
    modalSearchInput.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        ...
        window.location.href = (window.__HUGO_BASE__ || '') + '/search/?query=' + encodeURIComponent(modalSearchInput.value.trim());
      }
    });
  ...
  function displayPreview() {
    ...
    if (searchHits.length >= 5) {
      ...
      const a = createElement('a', {
        ...
        attrs: {href: (window.__HUGO_BASE__ || '') + `/search/?query=${encodeURIComponent(modalSearchInput.value)}`},
