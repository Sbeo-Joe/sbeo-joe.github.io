document.addEventListener('DOMContentLoaded', function() {
  ...
  function buildSearchUrl(preserveTaxonomy = false) {
    ...
    return (window.__HUGO_BASE__ || '') + `/search/?${params.toString()}`;
  }
    ...
    if (appendHeader) {
      ...
      const taxonomies = Object.keys(category1).toSorted()
        ...
        .map(key => ({
          ...
          icon: 'icon-file',
          href: (window.__HUGO_BASE__ || '') + `/search/?category1=${category1Name}&category2=${category1[key]['name']}`,
    ...
    if (appendHeader) {
      ...
      if (category1Name) {
        taxonomy = {
          ...
          href: (window.__HUGO_BASE__ || '') + `/search/?category1=${category1Name}`,
      ...
      if (!hasSingleTag) {
        const taxonomies = tagNames.toSorted()
          .map(tag => ({
            text: tag,
            icon: 'icon-tag',
            href: (window.__HUGO_BASE__ || '') + `/search/?tags=${tag}`,
  ...
  function displayPagination({cur, pages, prev = null, next = null}) {
    ...
    const createPageUrl = (page) => {
      ...
      return (window.__HUGO_BASE__ || '') + `/search/?${newParams.toString()}#pagination-anchor`;
    };
